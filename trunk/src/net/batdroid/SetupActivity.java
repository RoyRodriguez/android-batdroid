...
//import android.tether.system.Configuration;
  ...
  // import android.text.TextWatcher;
   ...
   import android.view.SubMenu;
		...
		/**
		* Class that handles the loading/changing/updating of configuration settings
		*/
   ...
   /**
   * Reads in settings from setupview.xml
   */
       @Override
       public void onCreate(Bundle savedInstanceState) {
        ...
        // Init CurrentSettings - TODO ? Don't we need to getString?
         this.currentSSID = this.application.settings.getString("ssidpref", "potato");//"ssidpref";//this.application.settings.getString("ssidpref", "batman");
         this.currentIpAddress = "10.130.1.254";
         this.currentNetmask = "255.255.255.0";
         this.currentChannel =this.application.settings.getString("channelpref", "1"); //"channelpref";// this.application.settings.getString("channelpref", "11");
         this.currentLAN = this.application.settings.getString("lannetworkpref", this.application.DEFAULT_LANNETWORK);//"lannetworkpref";//this.application.settings.getString("lannetworkpref", this.application.DEFAULT_LANNETWORK);
        ...
        // SSID-Validation
         ...
         this.prefSSID.setOnPreferenceChangeListener(
      ...
      //Ip Validation
         ...
         this.prefIP.setOnPreferenceChangeListener(
                new Preference.OnPreferenceChangeListener(){
                   public boolean onPreferenceChange(Preference preference,
                   Object newValue) {
      ...
      //Ip Validation
         ...
         this.prefNetmask.setOnPreferenceChangeListener(
                new Preference.OnPreferenceChangeListener(){
                   public boolean onPreferenceChange(Preference preference,
                   Object newValue) {
   ...
   /**
   * Displays dialog when restarting BATDROID
   */
       @Override
       protected Dialog onCreateDialog(int id) {
         if (id == ID_DIALOG_RESTARTING) {
            progressDialog = new ProgressDialog(this);
            progressDialog.setTitle("Restart B.A.T.D.R.O.I.D");
   ...
   /**
   * Calls the method to update the configurations with the new values
   */
      ...
      Handler restartingDialogHandler = 
          new Handler(){
      ...
      Handler displayToastMessageHandler = 
          new Handler() {
             public void handleMessage(Message msg) {
               if (msg.obj != null) {
   ...
   /**
   *	Method to update the configuration of BATDROID with the new values
   * @param sharePreferences
   * @param key The id of the value to change (eg. ssidpref)
   */
                ...
                new Runnable(){
                   public void run(){
                     ...
                     if (key.equals("ssidpref")) {
                        ...
                        if (SetupActivity.this.currentSSID.equals(newSSID) == false) {
                           ...
                           try{
                              if (application.coretask.isNatEnabled() /*&& application.coretask.isProcessRunning("bin/dnsmasq")*/)
                              {
                                 ...
                                 SetupActivity.this.application.restartBatDroid();
                        ...
                        String newIP = sharedPreferences.getString("ippref", "10.130.1.254");
      ...
      Handler  setWifiPrefsEnableHandler = 
          new Handler() {
             public void handleMessage(Message msg) {
               PreferenceGroup wifiGroup = (PreferenceGroup)findPreference("wifiprefs");
	...
	/**
	*	Validate the new SSID name, to determine all characters are valid.
	* @param newSSID SSID name to be validated
	*/
   	...
   	/**
   	*	Validates the IP address to determine if it has invalid characters
   	* @param newIP IP address to be validated
   	*/
   	...
   	/**
   	*	Validates the netmask to determine if it has invalid characters
   	* @param newNetmask netmask to be validated
   	*/
